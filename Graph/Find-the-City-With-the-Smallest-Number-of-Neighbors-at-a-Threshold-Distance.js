...
var findTheCity = function (n, edges, distanceThreshold) {
    ...
    function dijkstra(node) {
        ...
        while (heap.size()) {
            ...
            let [n, d] = heap.dequeue();

            if (distance[n] < d) continue;
            distance[n] = d;

            for (let [neighbor, dist] of (graph[n] || [])) {
                heap.enqueue([neighbor, d + dist]);
            }
    ...
    let minCount = n, minNode = -1;

    for (let i = 0; i < n; i++) {
        let distances = dijkstra(i);
        let count = 0;

        for (let j = 0; j < n; j++) {
            if (distances[j] !== 0 && distances[j] <= distanceThreshold) count++;
        }
