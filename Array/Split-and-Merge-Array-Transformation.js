...
var minSplitMerge = function (nums1, nums2) {
    ...
    let targetKey = nums2.join('#');
    let queue = [[nums1, 0, nums1.join('#')]], qi = 0;
    let visited = new Set();

    while (qi < queue.length) {
        let [nums, op, k] = queue[qi++];

        if (k === targetKey) return op;

        for (let i = 0; i < n; i++) {
            for (let j = i + 1; j <= n; j++) {

                let insert = nums.slice(i, j);
                let remaining = [...nums.slice(0, i), ...nums.slice(j)];
