 ...
 * @return {number}
 */
var minimumCoinsMemo = function (prices) {
    ...
    function dfs(index) {
        ...
        for (let i = index + 1; i <= Math.min(prices.length, index + index + 2); i++) {
            min = Math.min(min, prices[index] + dfs(i));
        }
...
var minimumCoins = function (prices) {
    ...
    for (let i = n - 1; i >= 0; i--) {
        ...
        for (let j = i + 1; j <= Math.min(n, 2 * i + 2); j++) {
            min = Math.min(min, prices[i] + dp[j]);
        }
