...
class Solution:
    def minCost(self, grid: list[list[int]]) -> int:
        ...
        # @cache
        def dfs(row, col, xor):
            nxor = grid[row][col] ^ xor
            
            if row == m-1 and col == n-1:
                return nxor

            if (row, col, xor) in memo:
                return memo[(row, col, xor)]
                
            mini = math.inf

            if col < n-1:
                mini = min(mini, dfs(row, col+1, nxor))   
             
            if row < m-1:
                mini = min(mini, dfs(row+1, col, nxor))

            memo[(row, col, xor)] = mini
