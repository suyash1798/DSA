...
class AuctionSystem:
    ...
    def addBid(self, userId: int, itemId: int, bidAmount: int) -> None:
        ...
        self.lastBid[(itemId, userId)] = bidAmount

    def updateBid(self, userId: int, itemId: int, newAmount: int) -> None:
        if (itemId, userId) in self.lastBid: 
            heapq.heappush(self.bids[itemId], [-newAmount, -userId])
            self.lastBid[(itemId, userId)] = newAmount      

    def removeBid(self, userId: int, itemId: int) -> None:
        if (itemId, userId) in self.lastBid:
            del self.lastBid[(itemId, userId)]

    def getHighestBidder(self, itemId: int) -> int:
        while len(self.bids[itemId]):
            amount, userId = self.bids[itemId][0]

            if (itemId, -userId) in self.lastBid and self.lastBid[(itemId, -userId)] == -amount:
                return -userId
