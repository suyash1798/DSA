...
class Router:
    ...
    def addPacket(self, source: int, destination: int, timestamp: int) -> bool:
        ...
        if len(self.seen) == self.limit:
            ...
            self.destination[d].popleft()
            self.seen.remove((s,d,t))
        
        self.queue.append([source, destination, timestamp])
        self.destination[destination].append([source, destination, timestamp])
    ...
    def getCount(self, destination: int, startTime: int, endTime: int) -> int:
        leftIndex, rightIndex = bisect.bisect_left(self.destination[destination], startTime, key=lambda x:x[2]), bisect.bisect_right(self.destination[destination], endTime, key=lambda x:x[2])
        
        return max(0, rightIndex - leftIndex)
